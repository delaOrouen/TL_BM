...
static void LCD_GPIOInit(void){
   unsigned long wait = 0;
    ...
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_N |= (1<<4);                        // make PN4, PN5 outputs
    GPIODIR_N |= (1<<5);
    GPIOAFSEL_N &= ~(1<<4);
    GPIOAFSEL_N &= ~(1<<5);
    GPIODEN_N |= (1<<4);                        // enable digital I/O on PN4, PN5
    GPIODIR_N |= (1<<5);
    ...
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_P |= (1<<0);                           // make PP0-1, PP3-4 outputs
    GPIODIR_P |= (1<<1);
    GPIODIR_P |= (1<<3);
    GPIODIR_P |= (1<<4);
    GPIOAFSEL_P &= ~(1<<0);                          // disable alternate functions
    GPIOAFSEL_P &= ~(1<<1);
    GPIOAFSEL_P &= ~(1<<3);
    GPIOAFSEL_P &= ~(1<<4);
    GPIODEN_P |= (1<<0);                             // enable digital I/O on PP0-1, PP3-4
    GPIODEN_P |= (1<<1);
    GPIODEN_P |= (1<<3);
    GPIODEN_P |= (1<<4);
    ...
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_Q |= (1<<0);                           // make PQ0, PQ2-3 outputs
    GPIODIR_Q |= (1<<2);
    GPIODIR_Q |= (1<<3);
    GPIOAFSEL_Q &= ~(1<<0);                           // disable alternate functions
    GPIOAFSEL_Q &= ~(1<<2);
    GPIOAFSEL_Q &= ~(1<<3);  
    GPIODEN_Q |= (1<<0);                            // enable digital I/O on PQ0, PQ2-3
    GPIODEN_Q |= (1<<2);
    GPIODEN_Q |= (1<<3);
}
